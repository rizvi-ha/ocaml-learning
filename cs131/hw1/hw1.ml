...
let rec computed_periodic_point eq f p x =
  ...
  in
  if eq x (apply_n_times f p x) then x else computed_periodic_point eq f p (f x)
;;
...
let filter_blind_alleys g =
  ...
  let rec build_terminable_symbols rules terminable_symbols =
    match rules with
    | (lhs, rhs) :: t ->
      if all_members_follow (is_terminal terminable_symbols) rhs
      then build_terminable_symbols t (lhs :: terminable_symbols)
  ...
  in
  let second_elements_equal (a, b) (c, d) = equal_sets b d in
  let terminable_set =
    snd (computed_fixed_point second_elements_equal bts_wrapper (snd g, []))
  in
  let rec remove_bad_rules rules =
    match rules with
    | [] -> []
    | (lhs, rhs) :: t ->
      if all_members_follow (is_terminal terminable_set) rhs
      then (lhs, rhs) :: remove_bad_rules t
